...
a.href = "#"
//a.href = chrome.runtime.getURL('ui/pages/options.html');
let link = document.createTextNode("WP PolyFill options");
a.id = "openOptionsLink"
a.appendChild(link);
optionlink.className = 'menu-item wppo_settings_menu'

let databaselink = document.createElement("li");
var b = document.createElement('a');
b.href = "#"
//b.target = "_self";
link = document.createTextNode("WP PolyFill database");
b.id = "openModalLink"
b.appendChild(link);
databaselink.className = 'menu-item wppo_database_menu'
...
function openOptionsPage(event) {
    const url = chrome.runtime.getURL("ui/pages/options.html");
    window.open(url)
}
...
async function translatedButton() {
    ...
    let wppoNavBar = document.createElement("div");
    let wppoNavBarCont = document.createElement("div");
    wppoNavBarCont.className = 'wppoNavBarCont'
    wppoNavBar.appendChild(wppoNavBarCont);
    wppoNavBar.className = "wppoNavBar";
    wppoNavBar.id = "wppoNavBar";
    if (divPaging != null && divProjects == null) {
        divGpActions.parentNode.insertBefore(wppoNavBar, divGpActions);
        let divNavBar = document.querySelector("div.wppoNavBarCont")
        if (is_pte) {
    ...
    WikiLink.href = 'https://github.com/lammersbjorn/WP-PolyFill/wiki'
    WikiLink.innerText = __("WP PolyFill Docs")
    WikiLink.className = 'menu-item-wppo_wiki'

    // Create a <select> element
    let dropdown = document.createElement("select");
    dropdown.name = "WPPO_select"
...
async function updateStyle(textareaElem, result, newurl, showHistory, showName, nameDiff, rowId, record, myHistory, my_checkpage, currstring, repl_array, prev_trans, old_status, showDiff) {
    ...
    if (markerpresent == null) {
        ...
        if (result.newText != "" && typeof result.newText != "undefined") {
            if (showName != true) {
                let markerimage = chrome.runtime.getURL("assets/images/warning-marker.png");
                if (current != null) {
