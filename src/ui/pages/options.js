...
scriptElm.src = chrome.runtime.getURL("ui/components/alert/cute-alert.js");
document.body.appendChild(scriptElm);
const template = `
    <h1>WP PolyFill - Options v${version}
    </h1>
    `;
...
link.type = "text/css";
link.rel = "stylesheet";
link.href = chrome.runtime.getURL("ui/components/alert/cute-alert.css");
...
chrome.storage.local.get(["apikey", "apikeyDeepl", "apikeyMicrosoft", "apikeyOpenAI", "OpenAIPrompt", "OpenAISelect", "OpenAITone", "OpenAItemp", "OpenAIWait", "DeepLWait", "reviewPrompt", "transsel", "destlang", "glossaryFile","glossaryFileSecond", "postTranslationReplace", "preTranslationReplace", "spellCheckIgnore", "showHistory", "showTransDiff", "glotDictGlos", "convertToLower", "DeeplFree", "TMwait", "bulkWait", "interXHR", "LtKey", "LtUser", "LtLang", "LtFree", "Auto_spellcheck", "Auto_review_OpenAI", "ForceFormal", "DefGlossary","WPPOscreenWidth","strictValidate", "autoCopyClip", "TMtreshold", "DownloadPath"], function (data) {
  ...
  //  }
    ...
    if (typeof data.WPPOscreenWidth == "undefined") {
        myScreenWidth = '90';
        myScreenWidthValue.value = myScreenWidth;
        rangeValue.textContent = myScreenWidth
    }
    else {
        myScreenWidth = data.WPPOscreenWidth;
        myScreenWidthValue.value = myScreenWidth;
        rangeValue.textContent = myScreenWidth
    }
    ...
    if ((parseFloat(OpenAItempVal)) >= 0 && (parseFloat(OpenAItempVal)) <= 2) {
        chrome.storage.local.set({
           ...
           // DownloadPath: DownloadTextbox.value,
            ...
            WPPOscreenWidth: theScreenWidthValue,
