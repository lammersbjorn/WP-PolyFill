   ...
   // console.debug("Checking index:", event)
    ...
    request.onsuccess = await function(event, result) {
        ...
        if (indexNames.contains('sourceCountry')){
            ...
            messageBox("error", 'Error the index does not exist in your DB!<br>Please make a backup of your database<br>Then follow the steps described in the Wiki to reset your database<br><a href="https://github.com/lammersbjorn/WP-PolyFill/wiki/13.-Fix-broken-local-database">https://github.com/lammersbjorn/WP-PolyFill/wiki/13.-Fix-broken-local-database</a>');
            result = false;
        }
    };
}
