...
function checkFormalPage(dataFormal) {
    ...
    if (dataFormal.length != 0 && dataFormal != "undefined") {
        ...
        let checkButton = document.querySelector(".wppoNavBarCont a.check_translation-button");
...
async function checkPage(postTranslationReplace, formal, destlang, apikeyOpenAI, OpenAIPrompt, spellcheckIgnore, showHistory) {
    ...
    if (is_pte) {
      ...
      //  console.debug("we are checking")
        ...
        var checkButton = await document.querySelector(".wppoNavBarCont a.check_translation-button");
            ...
            for (let e of document.querySelectorAll("tr.editor div.editor-panel__left div.panel-content")) {
               ...
               // console.debug("spanmissing:", spanmissing)
                ...
                if (spanmissing == null) {
                        ...
                        if (transtype == "single") {
                            ...
                            if (translatedText != "No suggestions" && translatedText != "") {
                                ...
                                if (replaced) {
                                    ...
                                    mypreview.classList.add("wppo-translated");
                                    repl_verb = result.repl_verb;
                    ...
                    console.debug("we have a record within the table without text")
                }
            }
        }
        else {
...
async function populateWithLocal(apikey, apikeyDeepl, apikeyMicrosoft, transsel, destlang, postTranslationReplace, preTranslationReplace, formal, convertToLower, DeeplFree) {
    ...
    let translateButton = document.querySelector(".wppoNavBarCont a.local-trans-button");
      ...
      //  console.debug("Populatewithlocal record:", record)
       ...
       // console.debug("transtype:", type,myCurr)
        ...
        if (pretrans != 'notFound') {
            ...
            currentClass.classList.add("wppo-translated");
            ...
            preview.classList.add("wppo-translated");
    ...
    translateButton = document.querySelector(".wppoNavBarCont a.local-trans-button");
...
async function processTM(myrecCount, destlang, TMwait, postTranslationReplace, preTranslationReplace, convertToLower, formal, spellCheckIgnore, TMtreshold, GlotPressBulkButton, FetchLiDelay, interCept) {
    ...
    for (let record of myrecCount) {
        ...
        if (counter == myrecCount.length) {
            // Translation completed
            translateButton = document.querySelector(".wppoNavBarCont a.tm-trans-button");
            translateButton.className += " translated";
            translateButton.innerText = __("Translated");
            progressbar = document.querySelector(".indeterminate-progress-bar");
            progressbar.style.display = "none";
...
async function populateWithTM(apikey, apikeyDeepl, apikeyMicrosoft, transsel, destlang, postTranslationReplace, preTranslationReplace, formal, convertToLower, DeeplFree, TMwait, postTranslationReplace, preTranslationReplace, convertToLower, spellCheckIgnore, TMtreshold, interCept) {
  //  const sortedRows = sortRowsArrayByRowAttribute();
   // myrecCount = sortedRows
    ...
    let GlotPressBulkButton = document.getElementById("bulk-actions-toolbar-bottom")
    // 19-06-2021 PSS added animated button for translation at translatePage
    let translateButton = document.querySelector(".wppoNavBarCont a.tm-trans-button");
...
async function mark_as_translated(row, current, translated, preview) {
    ...
    prevcurrentClass.classList.add("wppo-translated");
...
async function translatePage(apikey, apikeyDeepl, apikeyMicrosoft, apikeyOpenAI, OpenAIPrompt, transsel, destlang, postTranslationReplace, preTranslationReplace, formal, convertToLower, DeeplFree, completedCallback, OpenAISelect, openAIWait, OpenAItemp, spellCheckIgnore, deeplGlossary, OpenAITone, DeepLWait, openAiGloss) {
    ...
    // 19-06-2021 PSS added animated button for translation at translatePage
    let translateButton = document.querySelector(".wppoNavBarCont a.translation-filler-button");
    translateButton.innerText = __("Translate");
    //console.debug("Button classname:", translateButton.className);
    // 30-10-2021 PSS fixed issue #155 let the button spin again when page is already translated
                    ...
                    if (pretrans == "notFound") {
                       ...
                       // console.debug("we found a pretranslated!",pretrans,row,localRow)
                        ...
                        if (previewElem != null) {
                            ...
                            previewClass.classList.add("wppo-translated");
                        } else {
                        ...
                        waitForMyElement('.gp-js-message', 1000,"5271")
                            .then(element => {
                                ...
                                if (nothidden != null) {
                                    ...
                                    nothidden.classList.add("wppo-saved");
                                    ...
                                    preview.classList.remove("wppo-translated")
...
function saveLocal_2(bulk_timer) {
    ...
    let procRecId = processTableRecords('.wppo-translated', async function (preview) {

        var editorRow;
        var myNewRow;
        //console.debug("preview:",preview,preview.classList)
        // we only need to read the translated lines by wppo
        if (preview != null && preview.classList.contains("wppo-translated")) {
